...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for picture in orm.Picture.objects.all():
            if picture.image_type=="G":
                image = orm.GalleryImage(file=picture.file,
                                 filename=picture.filename,
                                 user=picture.user,
                                 upload_date=picture.upload_date,
                                 comment=picture.comment,
                                 order=picture.order,
                                 display_on_profile=picture.display_on_profile)
                image.save()
                
            elif picture.image_type=="C":
                image = orm.ProfileImage(file=picture.file,
                                 filename=picture.filename,
                                 user=picture.user,
                                 upload_date=picture.upload_date)
                image.save()
                image_owner = orm.UserProfile.objects.get(user=picture.user.id)
                image_owner.profile_image_uncropped = image
                image_owner.profile_image_cropped = image
                image_owner.save()
